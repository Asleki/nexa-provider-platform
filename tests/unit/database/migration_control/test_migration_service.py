...
def test_status_and_verify_are_read_only():
 l=MemoryMigrationLedger(False); s=MigrationControlService(ROOT,ROOT/'migration_manifest.json',l); assert s.status().pending_migrations==10; assert s.verify().ledger_state=='NOT_BOOTSTRAPPED'; assert not l.is_bootstrapped()
...
def test_structural_verify_checks_only_ledger_applied_migrations_when_repository_has_pending_entries():
    ...
    assert status.repository_migrations == 10
    assert status.applied_migrations == 6
    assert status.pending_migrations == 4

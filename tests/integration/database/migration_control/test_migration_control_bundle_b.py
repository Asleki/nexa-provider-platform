...
def test_fresh_database_status_is_read_only():
    ...
    assert result.repository_migrations==s.trusted_plan().migration_count
    assert result.applied_migrations==0
    assert result.pending_migrations==s.trusted_plan().migration_count
...
def test_plan_is_deterministic():
    ...
    assert [d.identity.sequence_number for d in a.forward_order]==list(range(1,a.migration_count+1))

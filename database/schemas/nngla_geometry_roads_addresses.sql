-- P006.7.5/P006.7.6 additive NNGLA geometry/road/address schema contract.
-- Contract only. Reuses the locked Bundle 14B PostGIS foundation and does not
-- register or execute a migration.
CREATE SCHEMA IF NOT EXISTS geography;

CREATE TABLE geography.nngla_geometry_authority_record (
    geometry_id text PRIMARY KEY CHECK (geometry_id ~ '^NG-GEO-[0-9]{6}$'),
    ...
    valid_to date,
    supersedes_geometry_id text,
    superseded_by_geometry_id text,
...
CREATE TABLE geography.nngla_survey_record (
    survey_id text PRIMARY KEY CHECK (survey_id ~ '^NG-SRV-[0-9]{6}$'),
    accuracy_class_code text NOT NULL,
    source_reference text NOT NULL,
    instrument_record_reference text,
    surveyor_approval_reference text,
    status text NOT NULL
);

CREATE TABLE geography.nngla_survey_control_point (
    survey_control_candidate_id text PRIMARY KEY,
    source_point_id text NOT NULL UNIQUE,
...
CREATE TABLE geography.nngla_road_reference_candidate (
    road_candidate_id text PRIMARY KEY CHECK (road_candidate_id ~ '^NG-RD-CAND-[0-9]{6}$'),
    ...
    addressing_eligible boolean NOT NULL,
    region_code text,
    source_basis text NOT NULL,
    runtime_effect_scope text NOT NULL
);
...
CREATE TABLE geography.nngla_road (
    road_id text PRIMARY KEY CHECK (road_id ~ '^NG-RD-[0-9]{6}$'),
...
CREATE TABLE geography.nngla_address (
    address_id text PRIMARY KEY CHECK (address_id ~ '^NG-ADR-[0-9]{6}$'),
    site_id text NOT NULL REFERENCES geography.nngla_addressable_site(site_id),
    road_id text,

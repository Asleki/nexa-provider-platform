...
test("governed irregular NoveGeo boundary is preserved as a MultiPolygon render plan", () => {
  ...
  assert.equal(plan.polygons.length, BUNDLED_WORLD_BOUNDARY_PUBLICATION.geometry.coordinates.length);
  assert.equal(plan.polygons.length, BUNDLED_WORLD_BOUNDARY_PUBLICATION.polygonCount);
  assert.equal(plan.polygons.length, 6);
  assert.ok(plan.polygons[0].rings[0].points.length > 100);
  ...
  assert.ok(uniqueX.size > 50);
  assert.ok(uniqueY.size > 50);
});

/** P006.7.11.15.9 - additive CITY_DISTRICT boundary/footprint + label overlay. */
...
import {
  ...
  createNoveGeoCityDistrictLabelCandidates,
  isNoveGeoCityDistrictMapItem,
} from "./city-district-anchor.js";

const DEFAULT_WIDTH = 640;
const MIN_WIDTH = 280;
const MIN_HEIGHT = 260;
const ASPECT = 0.68;
const ROLE = "novegeo-city-district-cartographic-canvas";
export const CITY_DISTRICT_MIN_ZOOM = 1.9;
...
function coordinatePoint(value) {
  if (!Array.isArray(value) || value.length < 2) throw new TypeError("CITY_DISTRICT coordinate must be [longitude,latitude]");
  const longitude = Number(value[0]);
  const latitude = Number(value[1]);
  if (!Number.isFinite(longitude) || !Number.isFinite(latitude)) throw new TypeError("CITY_DISTRICT coordinates must be finite");
  if (longitude < -180 || longitude > 180 || latitude < -90 || latitude > 90) throw new RangeError("CITY_DISTRICT coordinate outside EPSG:4326 bounds");
  return Object.freeze({ longitude, latitude });
}

export function normalizeCityDistrictPolygons(geometry) {
  if (!geometry || typeof geometry !== "object") throw new TypeError("CITY_DISTRICT geometry is required");
  ...
  throw new Error(`unsupported CITY_DISTRICT geometry type: ${geometry.type || "unknown"}`);
}

function drawPath(context, item, project) {
  const polygons = normalizeCityDistrictPolygons(item.geometry);
  let ringCount = 0;
  context.beginPath();
  for (const polygon of polygons) {
    if (!Array.isArray(polygon) || polygon.length < 1) throw new Error("CITY_DISTRICT polygon requires at least one ring");
    for (const ring of polygon) {
      if (!Array.isArray(ring) || ring.length < 4) throw new Error("CITY_DISTRICT ring requires at least four coordinates");
...
export function mountNoveGeoCityDistrictCartographicOverlay(documentRef, {
  boundaryPublication,
  cityDistrictItems = [],
  ...
  const items = assertPublishedNoveGeoCityDistrictSubset(cityDistrictItems);
  ...
  const render = () => {
    ...
    if (!context) throw new Error("CITY_DISTRICT cartographic overlay requires Canvas 2D");
    ...
    let ringCount = 0;
    if (navigation.zoom >= CITY_DISTRICT_MIN_ZOOM) {
      context.save();
      ...
      for (const item of items) {
        if (!isNoveGeoCityDistrictMapItem(item)) continue;
    ...
    const candidates = navigation.zoom >= CITY_DISTRICT_MIN_ZOOM
      ? createNoveGeoCityDistrictLabelCandidates(items, { readRuntime })
      : Object.freeze([]);
    ...
    container.dataset.novegeoCityDistrictCartographyVersion = "1";
    container.dataset.novegeoCityDistrictCartographyStatus = "RENDERED";
    container.dataset.novegeoCityDistrictCount = String(items.length);

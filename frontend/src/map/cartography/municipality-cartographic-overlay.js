/** P006.7.11.15.9.1 - additive MUNICIPALITY boundary + label overlay. */
...
import {
  ...
  createNoveGeoMunicipalityLabelCandidates,
  isNoveGeoMunicipalityMapItem,
} from "./municipality-anchor.js";

const DEFAULT_WIDTH = 640;
const MIN_WIDTH = 280;
const MIN_HEIGHT = 260;
const ASPECT = 0.68;
const ROLE = "novegeo-municipality-cartographic-canvas";
export const MUNICIPALITY_MIN_ZOOM = 1.7;
...
function coordinatePoint(value) {
  if (!Array.isArray(value) || value.length < 2) throw new TypeError("MUNICIPALITY coordinate must be [longitude,latitude]");
  const longitude = Number(value[0]);
  const latitude = Number(value[1]);
  if (!Number.isFinite(longitude) || !Number.isFinite(latitude)) throw new TypeError("MUNICIPALITY coordinates must be finite");
  if (longitude < -180 || longitude > 180 || latitude < -90 || latitude > 90) throw new RangeError("MUNICIPALITY coordinate outside EPSG:4326 bounds");
  return Object.freeze({ longitude, latitude });
}

export function normalizeMunicipalityPolygons(geometry) {
  if (!geometry || typeof geometry !== "object") throw new TypeError("MUNICIPALITY geometry is required");
  ...
  throw new Error(`unsupported MUNICIPALITY geometry type: ${geometry.type || "unknown"}`);
}

function drawMunicipalityPath(context, item, project) {
  const polygons = normalizeMunicipalityPolygons(item.geometry);
  let ringCount = 0;
  context.beginPath();
  for (const polygon of polygons) {
    if (!Array.isArray(polygon) || polygon.length < 1) throw new Error("MUNICIPALITY polygon requires at least one ring");
    for (const ring of polygon) {
      if (!Array.isArray(ring) || ring.length < 4) throw new Error("MUNICIPALITY ring requires at least four coordinates");
...
export function mountNoveGeoMunicipalityCartographicOverlay(documentRef, {
  boundaryPublication,
  municipalityItems = [],
  ...
  const municipalities = assertPublishedNoveGeoMunicipalitySubset(municipalityItems);
  ...
  const render = () => {
    ...
    if (!context) throw new Error("MUNICIPALITY cartographic overlay requires Canvas 2D");
    ...
    let ringCount = 0;
    if (navigation.zoom >= MUNICIPALITY_MIN_ZOOM) {
      context.save();
      context.fillStyle = "rgba(148,163,184,0.025)";
      context.strokeStyle = "rgba(148,163,184,0.78)";
      context.lineWidth = 1;
      context.lineJoin = "round";
      for (const item of municipalities) {
        if (!isNoveGeoMunicipalityMapItem(item)) continue;
        const receipt = drawMunicipalityPath(context, item, project);
    ...
    const candidates = navigation.zoom >= MUNICIPALITY_MIN_ZOOM
      ? createNoveGeoMunicipalityLabelCandidates(municipalities, { readRuntime })
      : Object.freeze([]);
    ...
    container.dataset.novegeoMunicipalityCartographyVersion = "1";
    container.dataset.novegeoMunicipalityCartographyStatus = "RENDERED";
    container.dataset.novegeoMunicipalityCount = String(municipalities.length);
    container.dataset.novegeoMunicipalityLabelCount = String(labelReceipt.renderedCount);

    latestReceipt = Object.freeze({
      status: "RENDERED",
      overlayVersion: 1,
      municipalityCount: municipalities.length,
      renderedMunicipalityIds: Object.freeze(municipalities.map((item) => item.subjectId)),
      polygonPartCount,
      ringCount,
  ...
  return Object.freeze({
    ...
    disconnect() {
      ...
      if (container.dataset) {
        delete container.dataset.novegeoMunicipalityCartographyVersion;
        delete container.dataset.novegeoMunicipalityCartographyStatus;
        delete container.dataset.novegeoMunicipalityCount;
        delete container.dataset.novegeoMunicipalityLabelCount;
      }
    },
  });
}

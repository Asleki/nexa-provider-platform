/** P006.7.11.15.9.1 - additive PWA bridge for governed MUNICIPALITYs. */
...
async function waitForMapCanvas(documentRef, windowRef, { attempts = 120, delayMs = 100 } = {}) {
  ...
  throw new Error("novegeo_municipality_map_canvas_timeout");
}

export function installNoveGeoMunicipalityMapExperience({
  ...
  const runRefresh = async () => {
    ...
    if (!resolved) {
      page.dataset.novegeoMunicipalityMapStatus = "DEGRADED";
      return Object.freeze({ status: "DEGRADED", reason: "live_api_endpoint_unavailable" });
    }
    const token = ++generation;
    page.dataset.novegeoMunicipalityMapStatus = "LOADING";
    try {
  ...
  return Object.freeze({
    ...
    disconnect() {
      ...
      const page = documentRef.querySelector?.(".novegeo-feature-page");
      if (page?.dataset) {
        delete page.dataset.novegeoMunicipalityMapStatus;
        delete page.dataset.novegeoMunicipalityMapCount;
      }
    },
  });
}
...
export function installNoveGeoMapExtension(options = {}) {
  return installNoveGeoMunicipalityMapExperience(options);
}

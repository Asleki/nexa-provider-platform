/** P006.7.11.15.9 - additive PWA bridge for governed TOWN. */
...
async function waitForMapCanvas(documentRef, windowRef, { attempts = 120, delayMs = 100 } = {}) {
  ...
  throw new Error("novegeo_town_map_canvas_timeout");
}

export function installNoveGeoTownMapExperience({
  ...
  const runRefresh = async () => {
    ...
    if (!resolved) {
      page.dataset.novegeoTownMapStatus = "DEGRADED";
      return Object.freeze({ status: "DEGRADED", reason: "live_api_endpoint_unavailable" });
    }
    const token = ++generation;
    page.dataset.novegeoTownMapStatus = "LOADING";
    try {
  ...
  return Object.freeze({
    ...
    disconnect() {
      ...
      const page = documentRef.querySelector?.(".novegeo-feature-page");
      if (page?.dataset) {
        delete page.dataset.novegeoTownMapStatus;
        delete page.dataset.novegeoTownMapCount;
      }
    },
  });
}
...
export function installNoveGeoMapExtension(options = {}) {
  return installNoveGeoTownMapExperience(options);
}
